 ...
 * token usage to Stripe for billing purposes with OpenRouter.
 *
 * OpenRouter exposes an OpenAI-compatible API, so the standard OpenAI SDK is
 * used with `baseURL` pointed at https://openrouter.ai/api/v1. The meter
 * detects OpenRouter responses and bills them under the underlying vendor
 * model (e.g. `anthropic/claude-sonnet-4`), not under `openai/...`.
 */
...
// Initialize the standard OpenAI client against OpenRouter (no wrapper needed!)
...
async function sampleBasicChatCompletion() {
  ...
  // Meter the response - auto-detects it's OpenRouter and bills
...
async function sampleStreamingChatCompletion() {
  const stream = await openrouter.chat.completions.create({
    ...
    max_tokens: 50,
    stream: true,
    // OpenRouter sends usage in the final chunk when include_usage is set
    stream_options: {include_usage: true},
  });

  // Wrap the stream for metering - OpenRouter streams use the OpenAI wrapper
...
async function sampleModelVariant() {
  ...
  // Billed as meta-llama/llama-3.1-8b-instruct
  meter.trackUsage(response, STRIPE_CUSTOMER_ID);
